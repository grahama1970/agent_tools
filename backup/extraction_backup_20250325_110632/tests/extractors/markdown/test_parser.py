"""
Tests for markdown-it parser functionality.

This module tests the markdown-it parser's ability to:
- Convert markdown to hierarchical JSON
- Extract code blocks, images, tables, and lists
- Build and flatten section hierarchies
- Process markdown files with proper metadata
"""
...
# Import the proper token counter
try:
    from agent_tools.dualipa.utils.spacy_utils import count_tokens, get_spacy_model
except ImportError:
    try:
        # Try alternative import path
        from agent_tools.utils.spacy_utils import count_tokens, get_spacy_model
    except ImportError:
        # Fallback to simple tokenization if spacy utils aren't available
        def count_tokens(text: str) -> int:
            """Simple fallback token counter that splits on whitespace."""
            return len(text.split())
        
        def get_spacy_model(*args, **kwargs):
...
try:
    ...
    raise ImportError(f"Required markdown-it parser modules not available: {e}. Fix the dependencies to run these tests.")
...
def markdown_file():
    ...
    with tempfile.NamedTemporaryFile(mode='w+', suffix='.md', delete=False) as f:
        ...
        file_path = Path(f.name)  # Convert to Path object
...
def test_markdown_it_availability():
    ...
    assert MARKDOWN_IT_AVAILABLE, "markdown-it-py is available but not properly initialized"
...
def test_extract_tables():
    ...
    if not MARKDOWN_IT_AVAILABLE:
        pytest.fail("markdown-it not available")
...
def test_file_metadata():
    ...
    try:
        ...
        assert file_info["filename"] == file_path.name
        assert file_info["extension"] == ".md"
        assert file_info["directory"] == str(file_path.parent)
...
def test_process_markdown_file(markdown_file):
    ...
    # Check file info
    file_info = result["document"]["file_info"]
    assert file_info["path"] == str(markdown_file)
    assert "token_count" in file_info
    assert file_info["token_count"] > 0
...
def test_get_flattened_markdown_content(markdown_file):
    ...
    if not MARKDOWN_IT_AVAILABLE:
        pytest.fail("markdown-it-py not available. Install it to run these tests.")
...
def test_token_counting_accuracy():
    """Test that token counting is consistent with spaCy tokenization."""
    # Use a real markdown file from test_repos
    project_root = Path("/home/grahama/workspace/experiments/agent_tools")
    test_file = project_root / "test_repos" / "react" / "packages" / "react-devtools-core" / "README.md"
    
    with open(test_file) as f:
        markdown_text = f.read()
    
    # Get token count using spaCy
    token_count = count_tokens(markdown_text)
    
    # Print token count for debugging
    print(f"Token count from spaCy: {token_count}")
    
    # Basic validation - ensure we found tokens
    assert token_count > 0, "Should find tokens in the markdown file"
    
    # Test specific sections if needed
    sections = ["# Introduction", "## Usage", "Simple paragraph text"]
    for section in sections:
        section_tokens = count_tokens(section)
        print(f"Token count for '{section}': {section_tokens}")
        assert section_tokens > 0, f"Should find tokens in section: {section}"
