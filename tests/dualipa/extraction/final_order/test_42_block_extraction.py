...
def test_extract_python_code_blocks():
    ...
    if setup_file.exists():
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Initialize stats
            stats = initialize_stats_dict(source=str(setup_file), output_dir=Path(output_dir))
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Initialize stats
        stats = initialize_stats_dict(source=str(api_file), output_dir=Path(output_dir))
...
def test_extract_javascript_code_blocks():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Initialize stats
        stats = initialize_stats_dict(source=str(js_file), output_dir=Path(output_dir))
...
def test_cross_language_extraction():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for lang, file_path in test_files:
            ...
            # Initialize stats
            stats = initialize_stats_dict(source=str(file_path), output_dir=Path(output_dir))
