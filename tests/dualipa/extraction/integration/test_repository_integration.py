...
This module tests the integration of various extraction components:
1. Repository cloning and setup
2. Multi-language extraction
...
try:
    ...
    raise ImportError(f"Required integration modules not available: {e}. Fix the dependencies to run these tests.")

# Only run tests if imports are available
pytestmark = pytest.mark.skipif(not HAS_DEPENDENCIES, reason="Required integration modules not available")
