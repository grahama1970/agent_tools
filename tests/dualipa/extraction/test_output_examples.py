...
""")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                # For script files, we should check for files in the output directory
                # rather than code blocks, since the entire file is the "block"
                files_dir = Path(temp_dir) / "files" / "code" / "python"
                
                # Check if the files directory exists and contains our setup.py
                if files_dir.exists():
                    extracted_files = list(files_dir.glob("*.py"))
                    print(f"Found {len(extracted_files)} extracted files")
                    
                    # If we found files, consider the test passed
                    if extracted_files:
                        for file_path in extracted_files:
                            print(f"Found extracted file: {file_path}")
                        assert len(extracted_files) > 0, "Should extract at least one file"
                        return
                
                # If we didn't find files in the expected location, check for blocks
                blocks_dir = Path(temp_dir) / "blocks" / "code" / "python"
                all_blocks = []
                
                if blocks_dir.exists():
                    all_blocks = list(blocks_dir.glob("*.py"))
                    print(f"Found {len(all_blocks)} blocks")
                    
                    for block_file in all_blocks:
                        with open(block_file, 'r') as bf:
                            content = bf.read()
                            print(f"Block content preview: {content[:100]}...")
                
                # Check the output directory structure to help debug
                ...
                # The test passes if we either found files or blocks
                assert len(all_blocks) > 0 or stats.get("total_files", 0) > 0, "Should extract either files or blocks"
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if "javascript" in test_files:
                    ...
                    try:
                        ...
                        # For script files, we should check for files in the output directory
                        # rather than code blocks, since the entire file is the "block"
                        files_dir = output_dir / "files" / "code" / lang
                        
                        # Check if the files directory exists and contains our file
                        if files_dir.exists():
                            extracted_files = list(files_dir.glob("*.*"))
                            print(f"Found {len(extracted_files)} extracted files")
                            ...
                            if extracted_files:
                                for file_path in extracted_files:
                                    print(f"Found extracted file: {file_path}")
                                assert len(extracted_files) > 0, f"Should extract at least one file for {lang}"
                                return
                        ...
                        # The test passes if we either found files or blocks
                        assert len(all_blocks) > 0 or stats.get("total_files", 0) > 0, f"Should extract either files or blocks for {lang}"
