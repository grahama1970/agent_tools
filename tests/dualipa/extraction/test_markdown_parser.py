...
def test_process_markdown_file(real_markdown_content):
    ...
    if not MARKDOWN_IT_AVAILABLE:
        pytest.fail("markdown-it-py not available")
    ...
    try:
        ...
        pytest.fail(f"Error processing markdown file: {e}")
...
def test_get_markdown_files():
    ...
    if not os.path.exists(REPO_ROOT):
        pytest.fail("Test repositories directory not found")
    ...
    try:
        ...
        if os.path.exists(rust_repo):
            ...
            pytest.fail("Rust-analyzer repository not found")
    
    except Exception as e:
        pytest.fail(f"Error finding markdown files: {e}")
...
def test_extract_code_blocks_from_documentation():
    ...
    if not doc_paths:
        pytest.fail("No documentation markdown files found")
    ...
    try:
        ...
        pytest.fail(f"Error extracting code blocks from documentation: {e}")

def test_advanced_markdown_parsing():
    """Test parsing complex markdown documents."""
    if not MARKDOWN_IT_AVAILABLE:
