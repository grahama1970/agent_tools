...
def test_js_function_extraction():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Extract blocks
        block_count = _extract_js_ts_blocks(js_file, js_content, output_dir, stats, language="javascript")
...
def test_ts_class_extraction():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Extract blocks
        block_count = _extract_js_ts_blocks(ts_file, ts_content, output_dir, stats, language="typescript")
...
def test_tsx_component_extraction():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Extract blocks
        block_count = _extract_js_ts_blocks(tsx_file, component_content, output_dir, stats, language="tsx")
        
        # Verify extraction
        assert block_count > 0, "Should extract at least one block"
        assert stats["code_blocks"] > 0, "Should increment code_blocks in stats"
        assert "tsx" in stats["languages"], "Should track TSX in languages"
        ...
        block_files = list(blocks_dir.glob("*.tsx")) + list(blocks_dir.glob("*.ts"))
        assert len(block_files) > 0, "Should create block files"
