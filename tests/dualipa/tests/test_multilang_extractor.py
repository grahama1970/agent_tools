...
js_ts_extractor_path = Path(__file__).parent.parent.parent.parent / "agent_tools" / "dualipa" / "scripts" / "js_ts_extractor.py"
...
def test_typescript_extraction(sample_files, temp_dir):
    ...
    # Check inheritance information in class declaration
    for decl in declarations:
        if decl['type'] == 'class_declaration' and decl['name'] == 'Employee':
            assert 'implements User' in decl.get('inheritance', '')
