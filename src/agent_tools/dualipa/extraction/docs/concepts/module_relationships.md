...
graph TD
    %% Core Utilities
    B[language_detection.py] --> C[utils.py]
    D[github_utils.py] --> C
    
    %% Tree-sitter and Parsing
    T[tree_sitter_hierarchy.py] --> B
    E[markdown_parser.py] --> B
    
    %% Language-Specific Extractors
    P[python_extractor.py] --> T
    J[js_ts_extractor.py] --> T
    M[markdown_extractor.py] --> E
    
    %% Code Hierarchy
    H[code_hierarchy.py] --> T
    H --> P
    H --> J
    H --> M
    
    %% Main Extractor
    A[code_extractor.py] --> B
    A --> D
    A --> H
    A --> P
    A --> J
    ...
    K[verification/verify_code_blocks.py] --> H
    K --> C
```
...
1. **Core Utilities**
   ...
   - `language_detection.py`: Language detection from file extensions and content
   - `github_utils.py`: GitHub repository handling and file discovery

2. **Parsing Infrastructure**
   - `tree_sitter_hierarchy.py`: Tree-sitter based code parsing
   - `markdown_parser.py`: Markdown parsing and block extraction

3. **Language-Specific Extractors**
   - `python_extractor.py`: Python code block extraction
   - `js_ts_extractor.py`: JavaScript/TypeScript extraction
   - `markdown_extractor.py`: Markdown code block extraction

4. **Code Organization**
   - `code_hierarchy.py`: Code structure and relationships
   - `verification/verify_code_blocks.py`: Code block validation

5. **Main Extraction**
   - `code_extractor.py`: Main extraction orchestration
   - `pipeline.py`: End-to-end extraction pipeline

## Key Function Relationships

### language_detection.py
- `_get_language_for_file(file_path: Path) -> str | None`
  - Core language detection from file extension
  - Used by all extractors
  - Returns standardized language identifiers or None for unknown

### tree_sitter_hierarchy.py
- `parse_code_hierarchy(content: str, language: str) -> Dict`
  - Parses code into hierarchical structure
  - Used by language-specific extractors
  - Provides consistent AST representation

### code_hierarchy.py
- `build_code_hierarchy(file_path: Path, content: str) -> Dict`
  - Builds unified code hierarchy
  - Delegates to appropriate language extractor
  - Maintains consistent structure across languages

### code_extractor.py
- `extract_repository(repo_path, output_dir, extract_blocks)` → Main entry point
  - Uses → `github_utils.discover_files()`
  - Uses → `_get_language_for_file()`
  - Delegates to appropriate language extractor
  - Maintains extraction statistics

### verification/verify_code_blocks.py
- `verify_code_block(block: Dict, language: str) -> bool`
  - Verifies code block structure
  - Validates against schema
  - Ensures consistent metadata

## Testing Dependencies

The test suite follows a strict dependency order that matches the module relationships:
...
1. **Language Detection Dependencies**
   - Must be tested before any extraction
   - Required by all language-specific extractors
   - Core to file type identification

2. **Format Validation Dependencies**
   - Must be tested before block extraction
   - Ensures consistent output structure
   - Required by all extractors

3. **Block Verification Dependencies**
   - Must be tested before advanced features
   - Ensures block integrity
   - Required by integration tests

4. **Tree-sitter Dependencies**
   - Must be tested before language extractors
   - Provides core parsing functionality
   - Required by Python and JS/TS extraction

5. **Code Hierarchy Dependencies**
   - Must be tested after language extractors
