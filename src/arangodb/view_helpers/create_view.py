...
async def delete_analyzer_if_exists(db, analyzer_name):
...
async def create_analyzer(db, analyzer_config):
...
async def create_view(db, arango_config):
...
async def create_vector_index(db, collection_name):
